function [result]=WriteAnalyzeHdr(name,dim,siz,pre,lim,scale,offset,origin,descr,gradorg),
% Added gradorg field to the hdr by Ofer Pasternak.
...
%
if (nargin ~=1) & (nargin ~= 8) & (nargin ~= 9) & (nargin ~= 10)
...
%fwrite(pid,zeros(16,1),'char');                         %%%%%%%%%%%   WRITE the vector here
fwrite(pid,0,'float32');
fwrite(pid,gradorg,'float32');
