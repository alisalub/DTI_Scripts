...
SourceDir='D:\HLrawdata\HL12_DATA\rExport2X10_FP\rSource';

% find r*.nii file in this directory  - the scans after slice time
d1 = dir([relevantDir,'/r*FA*.nii']); %FA img 
FA_images=cell(length(d1),1);
d2 = dir([relevantDir,'/r*MD_FP*.nii']); %MD img
...
d9 = dir([SourceDir,'/*FA*']); %FA img -source folder 
...
for i = 1:length(d1)
    ...
    %L1_images{i}=fullfile(relevantDir,d3(i).name); % store data
        %L1_images{i}=[L1_images{i},',1']; %insert ',1' for later SPM use
...
cd('D:\Normaliztion_New')
copyfile('NewMeanFA_FP.nii','NewMeanFA_FPCOPY.nii')

cd(Path)
%Normaliztion stages via SPM:
matlabbatch{1}.spm.spatial.normalise.estwrite.subj.source = {Source_FA_images{j}};
matlabbatch{1}.spm.spatial.normalise.estwrite.subj.wtsrc = '';
matlabbatch{1}.spm.spatial.normalise.estwrite.subj.resample = {FA_images{j};MD_images{j}}; %L1_images{j};L2_images{j};L3_images{j};MeanB0_images{j};RD_images{j};;B0_images{j}
matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.template = {'D:\Normaliztion_New\NewMeanFA_FPCOPY.nii,1'};
matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.weight = ''; 
matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.smosrc = 0.4;
matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.smoref = 0.4;
...
cd('D:\Normaliztion_New')
delete('D:\Normaliztion_New\NewMeanFA_FPCOPY.nii');
