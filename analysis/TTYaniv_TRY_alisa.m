function Result=TTYaniv_TRY_alisa(Names, savename)
...
s7=input('Enter no. of s7 >');
Working_Path='/Volumes/SAMSUNG/MSc_PhD/DTI_Analysis/smoothed_allscans';


load_name=Names{1};
full_path_name=strcat(Working_Path,'\swPB',load_name,'_nii_regularized_MD_C_native_MD_C_trafo_FA.nii') ;
...
nii2=load_untouch_nii('F:\MSC\AlisaScansDTI\AlisaScriptsDTI\analysis\PBmask.nii');
mask=nii2.img;
...
for i=1:length(Names)
    load_name=Names{i};
full_path_name=strcat(Working_Path,'\sw',load_name,'_nii_regularized_MD_C_native_DWIs_MD_C_trafo_MD.nii') ;
    nii=load_untouch_nii(full_path_name);
...
nii2=load_untouch_nii('F:\MSC\AlisaScansDTI\AlisaScriptsDTI\analysis\ALmask.nii');
mask=nii2.img;
mask=mask(:,:,:);
%mask=mask(:,1:143,:);
mask(find(mask>0))=1;
