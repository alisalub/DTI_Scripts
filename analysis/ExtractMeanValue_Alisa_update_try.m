...
P_value_nii=load_nii('/Volumes/SAMSUNG 1/MSc_PhD/DTI_Analysis/smoothed_allscans/ALanovaFA1_ttest.nii'); %load Anova image
...
meanValuesRegionFA = cell(1+length(Subjects),20,max(img_Mice_clust_FA(:)-1));
...
meanValuesRegionMD = cell(1+length(Subjects),20,max(img_Mice_clust_MD(:)-1));
columns_titles = {'Subject','clustnum', 'Scan1', 'Scan2', 'Scan3', 'Scan4','Scan5','Scan6','%change2-1' ,'%change3-2' ,'%change4-3','%change5-4','%change6-5','%change6-1', 'Scan1-std', 'Scan2-std', 'Scan3-std', 'Scan4-std','Scan5-std','Scan6-std'};
...
for clust_FA=2:max(img_Mice_clust_FA(:)); %loop for each cluster
    ...
    for subjectInd = 1:length(Subjects) %loop for each subject
        ...
        meanValuesRegionFA{1+subjectInd,15,clust_FA-1} = std_scan(1,1);
        meanValuesRegionFA{1+subjectInd,16,clust_FA-1} = std_scan(1,2);
        meanValuesRegionFA{1+subjectInd,17,clust_FA-1} = std_scan(1,3);
        meanValuesRegionFA{1+subjectInd,18,clust_FA-1} = std_scan(1,4);
        meanValuesRegionFA{1+subjectInd,19,clust_FA-1} = std_scan(1,5);
        meanValuesRegionFA{1+subjectInd,20,clust_FA-1} = std_scan(1,6);
...
for clust_MD=2:max(img_Mice_clust_MD(:)); %loop for each cluster
    ...
    for subjectInd = 1:length(Subjects) %loop for each subject
        mean_scan=zeros(1,scannum); %scans for each subject
        mean_std=zeros(1,scannum); %scans-std for each subject
        count=0;
        for scan=subjectInd:length(Subjects):(scannum*length(Subjects)) %loop for each scan
            count=count+1;
        ...
        meanValuesRegionMD{1+subjectInd,15,clust_MD-1} = std_scan(1,1);
        meanValuesRegionMD{1+subjectInd,16,clust_MD-1} = std_scan(1,2);
        meanValuesRegionMD{1+subjectInd,17,clust_MD-1} = std_scan(1,3);
        meanValuesRegionMD{1+subjectInd,18,clust_MD-1} = std_scan(1,4);
        meanValuesRegionMD{1+subjectInd,19,clust_MD-1} = std_scan(1,5);
        meanValuesRegionMD{1+subjectInd,20,clust_MD-1} = std_scan(1,6);
        
    end % end for subjectInd
end

save('meanValuesRegionMD1.mat','meanValuesRegionMD');
save('meanValuesRegionFA1.mat','meanValuesRegionFA');